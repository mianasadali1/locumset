...
export default class PreferencesDropdown extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount = () => {
    ...
    console.log('end node module');
    const listItems = this.props.items;
    const defaultIndex = this.props.defaultIndex;
    for (var i = 0; i <= listItems.length - 1; i++) {
      if (listItems[i].id == defaultIndex) {
        this.state.defaultItem = i;
      }
    }
    if (this.state.defaultItem != '') {
      console.log('in if');
      this.setState({
        listItems,
        item: listItems[this.state.defaultItem]
      });
    } else {
      console.log('in else');
